...
const ClassicEnterpriseTemplate = ({
  body = '',
  design = {},
  profile = {},
  signature = {},
  ...
  const bodyDesign = {
    ...
    lineHeight: Number(design.lineHeight) || 1.65,
  };
  ...
  return (
    <div
      ...
      style={{
        ...
        '--enterprise-accent': accentColor,
      }}
    >
      {/* CLASSIC HEADER */}
      <header className="border-b-2 border-slate-900 pb-5 text-center">
        {hasValue(profile.fullName) && (
          <h1 className="text-[32px] font-bold uppercase leading-tight tracking-[0.12em] text-slate-950">
            {profile.fullName}
          </h1>
        )}

        {(hasValue(profile.currentJobTitle) || visibleSkills.length > 0) && (
          <p className="mx-auto mt-3 max-w-[165mm] text-[12px] font-semibold leading-5 text-slate-600">
      ...
      {/* DATE + RECIPIENT */}
      <section className="mt-7 grid grid-cols-[38mm_1fr] gap-8 border-b border-slate-200 pb-5">
        <div>
      ...
      <main className="mt-7">
        <Body
          ...
          className="
            ...
            prose-strong:text-[var(--enterprise-accent)]
            [&>*:first-child]:mt-0
            [&>*:last-child]:mb-0
          "
        />
...
export default ClassicEnterpriseTemplate;