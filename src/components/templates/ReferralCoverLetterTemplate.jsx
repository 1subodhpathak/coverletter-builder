...
const ReferralCoverLetterTemplate = ({
  body = '',
  design = {},
  profile = {},
  signature = {},
  ...
  const featuredSkills =
    visibleSkills.length > 0
      ? visibleSkills
      : ['Trusted Recommendation', 'Role Alignment', 'Relevant Experience'];
    ...
    >
      ...
      >
        ...
        <p className="mt-1.5 text-[10.5px] font-semibold leading-4 text-slate-700">
          {hasValue(referralName)
            ? `Naturally mentions the referral from ${referralName} while keeping the focus on the candidate’s value.`
            : 'Naturally mentions a referral while keeping the focus on the candidate’s value.'}
        </p>
      </section>
      ...
      <main className="mt-5 rounded-2xl border border-slate-100 bg-white px-4 py-4 shadow-[0_10px_26px_rgba(15,23,42,0.035)]">
        <Body
          ...
          className="
            ...
            prose-strong:text-[var(--referral-accent)]
            [&>*:first-child]:mt-0
            [&>*:last-child]:mb-0
          "
        />
...
export default ReferralCoverLetterTemplate;